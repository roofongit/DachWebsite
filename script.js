...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // ===========================
    // SIMPLE SESSION VIEW COUNTER
    // ===========================
    (function() {
        const counterEl = document.getElementById('visitCounter');

        // If not already counted this session, count it
        if (!sessionStorage.getItem('sessionViewCounted')) {
            sessionStorage.setItem('sessionViewCounted', 'true');
            sessionStorage.setItem('sessionViewCount', '1'); // Start at 1
        }

        // Read and display session counter
        let count = sessionStorage.getItem('sessionViewCount') || '0';
